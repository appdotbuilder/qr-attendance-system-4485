...
class AttendanceTest extends TestCase
{
    ...
    public function test_employee_can_check_out(): void
    {
        ...
        // Create attendance with proper check_in time that matches today
        $attendance = Attendance::create([
            'user_id' => $employee->id,
            'office_id' => $office->id,
            'check_in' => now()->startOfDay()->addHours(8), // 8 AM today
            'check_in_lat' => -6.2088,
        ...
        // Verify the attendance can be found by the getTodayActiveAttendance method
        $foundAttendance = $employee->getTodayActiveAttendance();
        $this->assertNotNull($foundAttendance, 'Active attendance should be found');
        ...
        // Refresh the attendance record from database
        $attendance = $attendance->fresh();
        
        $this->assertEquals('completed', $attendance->status, 'Attendance status should be completed after check-out');
        $this->assertNotNull($attendance->check_out, 'Check-out time should be set');
        $this->assertNotNull($attendance->work_duration, 'Work duration should be calculated');
    }
